...
const INTEGRATION_URL =
  process.env.NEXT_PUBLIC_INTEGRATION_URL || "https://upload.notaku.cloud";
...
export class OCRApiClient {
  ...
  static async uploadReceipt(
    ...
    try {
      ...
      console.log(
        `[Integration API] File: ${file.name} (${(file.size / 1024).toFixed(
          2
        )} KB)`
      );
      ...
      console.log(
        `[Integration API] Processing time: ${result.processing_time}`
      );
   ...
   */
  static async uploadPremiumReceipt(
    file: File,
    token?: string
  ): Promise<OCRResult> {
    try {
      ...
      console.log(
        `[Integration API] 🚀 Uploading to Premium Processing: ${uploadUrl}`
      );
      ...
      if (!response.ok) {
        const error = await response
          .json()
          .catch(() => ({ message: "Premium OCR upload failed" }));

        // Handle specific error cases
        if (response.status === 403) {
          throw new Error(
            "Premium subscription required. Please upgrade your account to use Premium OCR."
          );
        }
        ...
        throw new Error(
          error.message ||
            error.detail ||
            `Premium OCR failed: ${response.status}`
        );
      }
  ...
  static async checkStatus(jobId: string): Promise<JobStatus> {
    console.warn(
      "[Integration API] ⚠️ checkStatus() is deprecated - Integration Service is synchronous"
    );
    throw new Error(
  ...
  static async getResult(jobId: string): Promise<OCRResult> {
    console.warn(
      "[Integration API] ⚠️ getResult() is deprecated - Integration Service is synchronous"
    );
    throw new Error(
  ...
  static async pollStatus(
    ...
    console.warn(
      "[Integration API] ⚠️ pollStatus() is deprecated - Integration Service is synchronous"
    );
    throw new Error(
      "Status polling not supported. Integration Service processes receipts synchronously. " +
